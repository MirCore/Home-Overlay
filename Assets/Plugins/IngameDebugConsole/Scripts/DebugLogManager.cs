...
#pragma warning restore 0414
		...
		[SerializeField]
		private TMPro.TMP_Text commandSuggestionPrefab;
		...
		[SerializeField]
		private TMPro.TMP_InputField commandInputField;
		...
		[SerializeField]
		private TMPro.TMP_Text infoEntryCountText;
		[SerializeField]
		private TMPro.TMP_Text warningEntryCountText;
		[SerializeField]
		private TMPro.TMP_Text errorEntryCountText;
...
#pragma warning restore 0649
		...
		private List<TMPro.TMP_Text> commandSuggestionInstances;
		...
		private void Awake()
		{
			...
			commandSuggestionInstances = new List<TMPro.TMP_Text>( 8 );
			...
			if( enableSearchbar )
				searchbar.GetComponent<TMPro.TMP_InputField>().onValueChanged.AddListener( SearchTermChanged );
			else
			{
				searchbar = null;
				...
				for( int i = 0; i < suggestionsCount; i++ )
				{
					if( i >= visibleCommandSuggestionInstances )
					{
						if( i >= suggestionInstancesCount )
							commandSuggestionInstances.Add( (TMPro.TMP_Text) Instantiate( commandSuggestionPrefab, commandSuggestionsContainer, false ) );
						else
